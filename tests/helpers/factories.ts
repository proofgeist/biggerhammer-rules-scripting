...
async function getContractJobTitleId(contractId: string): Promise<string> {
  ...
  if (result.error) {
    throw new Error(`Failed to look up CJT for contract ${contractId}: ${result.error}`);
  }
...
export async function createTimeCard(params: {
  ...
  const result = await db.from(TCD__TimeCard).insert({
  ...
  if (result.error) throw new Error(`Failed to create TimeCard: ${result.error}`);
  return result.data;
}
...
export async function createClockTCL(params: {
  ...
  const result = await db.from(TCL__TimeCardLine).insert({
  ...
  if (result.error) throw new Error(`Failed to create Clock TCL: ${result.error}`);
  return result.data;
}
...
export async function getResultTCLs(timecardId: string) {
  ...
  console.log(`getResultTCLs: ${records.length} total records for TCD ${timecardId}`);
